...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: `How to Play Board Games | ${brand.siteName}`,
    description:
      "Clear, structured rules guides for popular board games. Learn the rules and start playing faster.",
  };
}

export default function HowToPlayIndexPage() {
  const config = getSiteVertical();
  if (config.slug !== "warhammer") redirect("/");

  const articles = listArticles("how-to-play");
      ...
      <main className="mx-auto max-w-4xl px-4 py-12 md:py-16">
        ...
        <header className="mb-10 border-b border-border pb-8">
          ...
          <p className="mt-3 text-lg text-muted-foreground">
            Clear rules guides so you can learn the game and start playing
            faster.
          </p>
        </header>
          ...
          <ul className="divide-y divide-border">
            {articles.map((article) => (
              <li key={article.slug} className="py-6 first:pt-0 last:pb-0">
                <article>
                  <Link
                    href={`/boardgames/how-to-play/${article.slug}`}
