...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: `Best Board Games | ${brand.siteName}`,
    description:
      "Curated lists of the best board games by category, player count, budget, and more.",
  };
}
...
export default function BestOfIndexPage() {
  ...
  const articles = listArticles("best");
      ...
      <main className="mx-auto max-w-4xl px-4 py-12 md:py-16">
        ...
        <header className="mb-10 border-b border-border pb-8">
          ...
          <p className="mt-3 text-lg text-muted-foreground">
            Curated lists of the best board games by category, player count, and
            budget.
          </p>
        </header>
          ...
          <ul className="divide-y divide-border">
            {articles.map((article) => (
              <li key={article.slug} className="py-6 first:pt-0 last:pb-0">
                <article>
                  <Link
                    href={`/boardgames/best/${article.slug}`}
